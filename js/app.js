...
let SERVICES=[],CASES=[],POSTS=[];

// State for lists
let casePage=1, CASES_PER=3, filteredCases=[], caseSort='newest', caseBrand='', caseType='', caseQuery='';
let postPage=1, POSTS_PER=4, filteredPosts=[], postSort='newest', postCategory='', postTag='', postQuery='';
    ...
    <section class="container">
      ...
      <div class="cases-grid">
        ${CASES.slice(0,3).map(c=>`
          <div class="case-card">
            <div class="case-thumb"><img src="${c.after||''}" alt="${c.title}"></div>
            <h3>${c.title}</h3>
...
function renderCases(){
  filteredCases=[...CASES];
  casePage=1; caseSort='newest'; caseBrand=''; caseType=''; caseQuery='';
  showCases();
}
...
function applyCaseFilters(){
  filteredCases = CASES.filter(c=>{
    ...
    const typeMatch = !caseType || c.deviceType.toLowerCase().includes(caseType);
...
function showCases(){
  const sorted=[...filteredCases].sort((a,b)=> caseSort==='newest' ? (new Date(b.date)-new Date(a.date)) : (new Date(a.date)-new Date(b.date)));
  ...
  $('#app').innerHTML=`
    <section class="container">
      ...
      <div class="controls">
        <label>Brand
          <select id="c-brand">
            ...
            ${brands.map(b=>`<option value="${b.toLowerCase()}" ${caseBrand===b.toLowerCase()?'selected':''}>${b}</option>`).join('')}
          </select>
        </label>
        <label>Device
          <select id="c-type">
            <option value="">All</option>
            ${types.map(t=>`<option value="${t.toLowerCase()}" ${caseType===t.toLowerCase()?'selected':''}>${t}</option>`).join('')}
          </select>
        </label>
        <label>Search <input id="c-q" placeholder="keywords" value="${caseQuery||''}"></label>
        <label>Sort
          <select id="c-sort">
            <option value="newest" ${caseSort==='newest'?'selected':''}>Newest</option>
            <option value="oldest" ${caseSort==='oldest'?'selected':''}>Oldest</option>
          </select>
        </label>
      </div>
      ...
      <div class="pager">
        ...
        <button id="nextCases" ${casePage>=totalPages?'disabled':''}>Next</button>
      </div>
      <a href="#home" class="btn" style="margin-top:12px">← Back</a>
    </section>`;

  $('#c-brand').addEventListener('change', e=>{caseBrand=e.target.value;applyCaseFilters();});
  $('#c-type').addEventListener('change', e=>{caseType=e.target.value;applyCaseFilters();});
  $('#c-q').addEventListener('input', e=>{caseQuery=e.target.value.toLowerCase();applyCaseFilters();});
  $('#c-sort').addEventListener('change', e=>{caseSort=e.target.value;showCases();});
  $('#prevCases')?.addEventListener('click', ()=>{casePage--;showCases();});
  $('#nextCases')?.addEventListener('click', ()=>{casePage++;showCases();});
}

// Case detail with slider + gallery + lightbox
function renderCaseDetail(slug){
  const c = CASES.find(x=>x.slug===slug);
  if(!c){ $('#app').innerHTML='<section class="container"><p>Case not found.</p></section>'; return; }
  $('#app').innerHTML=`
    ...
    <section class="container">
      ...
      <div class="gallery-grid">
        ${(c.gallery||[]).map((g,i)=>`
          <div class="card"><a href="#" class="lightbox-link" data-index="${i}" data-slug="${c.slug}"><img src="${g}" alt="Gallery image ${i+1}"></a></div>
        `).join('')}
      </div>
    </section>
...
function initLightbox(caseItem){
  ...
  let idx = 0;
  const open = (i)=>{ idx=i; img.src = caseItem.gallery[idx]; lb.classList.add('open'); };
  const close = ()=> lb.classList.remove('open');
  const next = ()=>{ idx=(idx+1)%caseItem.gallery.length; img.src=caseItem.gallery[idx]; };
  const prev = ()=>{ idx=(idx-1+caseItem.gallery.length)%caseItem.gallery.length; img.src=caseItem.gallery[idx]; };

  $$('.lightbox-link').forEach(a=>a.addEventListener('click',(e)=>{e.preventDefault(); open(parseInt(a.dataset.index,10));}));
...
function renderBlog(){
  filteredPosts=[...POSTS];
  postPage=1; postSort='newest'; postCategory=''; postTag=''; postQuery='';
  showPosts();
}

function applyPostFilters(){
  filteredPosts = POSTS.filter(p=>{
    const catMatch = !postCategory || (p.category||'').toLowerCase().includes(postCategory);
    const tagMatch = !postTag || (p.tags||[]).map(t=>t.toLowerCase()).includes(postTag);
...
function showPosts(){
  const sorted=[...filteredPosts].sort((a,b)=> postSort==='newest' ? (new Date(b.date)-new Date(a.date)) : (new Date(a.date)-new Date(b.date)));
    ...
    <section class="container">
      ...
      <div class="controls">
        <label>Category
          <select id="p-cat">
            <option value="">All</option>
            ${categories.map(c=>`<option value="${c.toLowerCase()}" ${postCategory===c.toLowerCase()?'selected':''}>${c}</option>`).join('')}
          </select>
        </label>
        <label>Tag
          <select id="p-tag">
            <option value="">All</option>
            ${tags.map(t=>`<option value="${t.toLowerCase()}" ${postTag===t.toLowerCase()?'selected':''}>${t}</option>`).join('')}
          </select>
        </label>
        <label>Search <input id="p-q" placeholder="keywords" value="${postQuery||''}"></label>
        <label>Sort
          <select id="p-sort">
            <option value="newest" ${postSort==='newest'?'selected':''}>Newest</option>
            <option value="oldest" ${postSort==='oldest'?'selected':''}>Oldest</option>
          </select>
        </label>
      </div>
      ...
      <div class="blog-grid">
        ${items.map(p=>`
          <div class="post-card">
            <h3><a href="#post/${p.slug}">${p.title}</a></h3>
            <div class="post-meta">${p.date} • ${(p.tags||[]).join(', ')}${p.category? ' • '+p.category : ''}</div>
            <p>${p.excerpt}</p>
            <a href="#post/${p.slug}" class="btn small">Read More</a>
          </div>`).join('')}
      </div>
      ...
      <div class="pager">
        ...
        <button id="nextPosts" ${postPage>=totalPages?'disabled':''}>Next</button>
      </div>
      <a href="#home" class="btn" style="margin-top:12px">← Back</a>
    </section>`;

  $('#p-cat').addEventListener('change', e=>{postCategory=e.target.value;applyPostFilters();});
  $('#p-tag').addEventListener('change', e=>{postTag=e.target.value;applyPostFilters();});
  $('#p-q').addEventListener('input', e=>{postQuery=e.target.value.toLowerCase();applyPostFilters();});
  $('#p-sort').addEventListener('change', e=>{postSort=e.target.value;showPosts();});
  $('#prevPosts')?.addEventListener('click', ()=>{postPage--;showPosts();});
  $('#nextPosts')?.addEventListener('click', ()=>{postPage++;showPosts();});
}

// Blog detail
function renderPostDetail(slug){
  const p = POSTS.find(x=>x.slug===slug);
  if(!p){ $('#app').innerHTML='<section class="container"><p>Post not found.</p></section>'; return; }
  $('#app').innerHTML=`
    <section class="container detail">
      <div class="card">
        <h2>${p.title}</h2>
...
function renderContact(){
  ...
  form.addEventListener('submit', (e)=>{
    ...
    if(!name || !email || !msg){ status.textContent = 'Please fill in all fields.'; return; }
    status.textContent = 'Message sent (demo). We will get back to you shortly.';
    form.reset();
  });
}
...
function initBASliders(){
  $$('.ba').forEach(el=>{
    ...
    const update = v => { after.style.clipPath = `inset(0 0 0 ${v}%)`; handle.style.left = v+'%'; };
    update(range.value);
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  const ham=$('#hamburger'),drawer=$('#drawer'),overlay=$('#overlay'),closeBtn=$('#close-drawer');
  const open=()=>{drawer.classList.add('open');overlay.classList.add('open');ham.classList.add('active');drawer.setAttribute('aria-hidden','false');}
  const close=()=>{drawer.classList.remove('open');overlay.classList.remove('open');ham.classList.remove('active');drawer.setAttribute('aria-hidden','true');}
